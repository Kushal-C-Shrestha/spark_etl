...
def create_query_tables(logger,output_dir, artists_df, recommendations_df, tracks_df):
    ...
    tracks_exploded=tracks_df.withColumn("id_artists_array", F.from_json(F.col("id_artists"), T.ArrayType(T.StringType()))).withColumn("artist_id", F.explode("id_artists_array")).select("id","artist_id")
